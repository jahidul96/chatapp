...
import { getLastMsg } from "../../firebase/fireStore/FirestoreFb";
    ...
    >
      <View style={styles.nameAvatorWrapper}>
        <Text style={styles.userAvator}>
          {data.username.length > 2 && data.username.slice(0, 1).toUpperCase()}
        </Text>
      </View>
      <View style={styles.userChatDetailsWrapper}>
        <View style={[styles.flexStyle, styles.nameWrapper]}>
          <Text style={styles.name}>{data.username.toUpperCase()}</Text>
        ...
        {lastMsg ? (
          <View style={styles.flexStyle}>
            <View style={styles.flexStyle}>
              {lastMsg?.photoUrl ? (
                <ImageIcon name="image" size={12} style={{ marginRight: 3 }} />
              ) : null}
